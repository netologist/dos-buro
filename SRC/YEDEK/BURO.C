...
void form_cagir(int referans)
{
	...
	switch (referans) {
		...
		case Form_PARA_KUTUSU :
			...
			if (access("data/hareket.dat", 0) == 0)
				kayit_no_al();
			else
				strcpy(&dataci[22], "000000");
			kasa_goruntusu();
			kayit_adresi = 0;
		...
		case Form_ODEME_KARTI :
			...
			break;
		case Form_KULLANICI :
			formgolge(5, 20, "bin/kullan.frm");
			formyukle(5, 20, "bin/kullan.frm");
			git_xy(0, 0);
			islem();
			break;
...
{
	int islemler, ch;
	char *veri = "0", *bellek, *menu_kontrol = "0";
	...
	switch (tablo[sayac].referans) {
		...
		case KMenu_ODEME_TURU :
			if (data[0] != '0' && data[0] != '1' && data[0] != '2') {
				...
				for (n = sayac+2; n < baslik->eleman_sayisi-1; n++)
...
/*--------------------------*/
	...
	ana_menu(); //*/
/*	clrscr();
	kalin_cerceve(1, 1, 13, 43, CYAN_ | A_BEYAZ);
	for (i = 0; i < 13; i++) {
		doldur(2+i, 2, ' ', 42, CYAN_);
		doldur(2+i, 5, ' ', 36, BEYAZ_);
	}
	harfler(1, 3, " MÅkellef Listesi ", CYAN_ | A_BEYAZ);
	ince_cerceve(2, 3, 11, 39, CYAN_ | GRI);
	cift_renk(3, 5, "    K o d    &≥      önvan (Ad)      ", KAHVE_ | A_BEYAZ, KAHVE_ | BEYAZ);
	ice_pen(4, 5, 11, 35);
	formyarat(10, 10, 6, 22, "bin/talinan.frm");
	getch();
	formyukle(9, 9, "bin/talinan.frm");*/
}
#endif